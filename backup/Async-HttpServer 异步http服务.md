...
private:
    ...
    void handle_request() {
          ...
          ::handle_request(req_, *res_ptr);
  
          auto self = shared_from_this();
          http::async_write(socket_,  *res_ptr,
              [self, res_ptr](beast::error_code ec, std::size_t) {
                  if (ec && ec != http::error::end_of_stream && ec != asio::error::eof) {
...
> 需要注意的是
handle_request() 函数中http::async_write 的第二个参数不能使用局部变量的原因是因为async_write 是个异步函数，在handle_request() 执行完成后，第二个参数已经被释放。async_write中的回调函数后台会访问，造成崩溃，所以使用智能指针的方式，让其在回调函数中依然有效，
